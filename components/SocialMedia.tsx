...
export function SocialMedia() {
  return (
    <motion.div
      ...
      className="fixed left-4 sm:left-6 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col gap-3"
    >
      {socials.map((social, index) => {
        ...
        return (
          <motion.a
            key={index}
            href={social.href}
            target={social.href.startsWith("tel") || social.href.startsWith("mailto") ? undefined : "_blank"}
            rel={social.href.startsWith("tel") || social.href.startsWith("mailto") ? undefined : "noopener noreferrer"}
            whileHover={{ scale: 1.1, x: 5 }}
            whileTap={{ scale: 0.95 }}
            className={`w-12 h-12 bg-gray-900 rounded-full flex items-center justify-center text-white shadow-lg transition-colors ${social.color}`}
